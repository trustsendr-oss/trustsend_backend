...
const BUSINESS_PAYOUT_ENDPOINT = 'POST /api/v1/business/mobile-money/payouts'
 ...
 * this request).
 */
export default class BusinessPayoutsController {
  async store({ business, businessAuthMethod, request, response, correlationId }: HttpContext) {
    ...
    const identity = {
      ...
      actorId: business.id,
      endpoint: BUSINESS_PAYOUT_ENDPOINT,
    }
  ...
  async show({ business, params, response }: HttpContext) {
    try {
      const txn = await MobileMoneyPayoutService.getByIdForInitiator(params.id, 'business', business.id)
      return response.ok({
        data: {
          payout_id: txn.id,
          status: txn.status,
  ...
  /**
   * GET /api/v1/business/mobile-money/payouts/:id/live-status
