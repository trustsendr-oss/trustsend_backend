...
const DEPOSIT_ENDPOINT = 'POST /api/v1/mobile-money/deposits'

export default class MobileMoneyDepositsController {
  /**
   * POST /api/v1/mobile-money/deposits
   * User deposits into their own wallet from mobile money — no agent involved.
   */
    ...
    try {
      ...
      const body = {
        data: {
          ...
          // The deposit is pending and the payer now has to authorise it — this is the moment
          // to tell them how. Comes from the same active-conf lookup already done above, so it
          // costs no extra provider call. null when the operator needs no payer action.
  ...
  /**
   * GET /api/v1/mobile-money/deposits/:id
   */
  async show({ auth, params, response }: HttpContext) {
    ...
    try {
      const txn = await MobileMoneyDepositService.getByIdForInitiator(params.id, 'user', user.id)
      return response.ok({
        data: {
          deposit_id: txn.id,
          status: txn.status,
  ...
  /**
   * GET /api/v1/mobile-money/deposits/:id/live-status
    ...
    try {
      ...
      const updated =
        live.status === 'COMPLETED' || live.status === 'FAILED'
          ? await MobileMoneyDepositService.confirmFromCallback(provider.name, txn.providerReferenceId!, live.status, {
      ...
      if (err.name === 'PawaPayRequestException') {
        return response.serviceUnavailable({ message: 'Mobile money provider unavailable, or has no record of this deposit yet, try again' })
      }
