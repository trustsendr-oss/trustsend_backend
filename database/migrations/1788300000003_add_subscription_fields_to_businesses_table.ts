...
export default class extends BaseSchema {
  ...
  async up() {
    this.schema.alterTable(this.tableName, (table) => {
      table.timestamp('plan_subscribed_at').nullable().comment('When the business last paid the one-time fee for its current plan')
      table
        .timestamp('plan_next_maintenance_billing_at')
        .nullable()
        .comment('Next time PlanService.chargeDueMaintenanceFees() should attempt this business\'s recurring plan fee — null if the plan has no maintenance_price')
      table
